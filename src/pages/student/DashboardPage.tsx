...
import LecturesTab from '../../components/dashboard/LecturesTab';
...
type DashboardTab = 'overview' | 'lectures';
...
export default function DashboardPage() {
  ...
  const tabs: { key: DashboardTab; label: string }[] = [
    ...
    { key: 'lectures', label: t.dashboard.tabs.lectures },
  ];
  ...
  return (
    <div>
      ...
      <div className="mb-8 flex gap-2 overflow-x-auto rounded-xl bg-white/80 p-1 shadow-sm">
        {tabs.map(({ key, label }) => (
          <button
            key={key}
            type="button"
            ...
            className={`shrink-0 rounded-lg px-5 py-2.5 text-sm font-semibold transition-colors ${
              tab === key ? 'bg-primary-blue text-white' : 'text-deep-navy/60 hover:text-primary-blue'
            }`}
          >
            {label}
          </button>
